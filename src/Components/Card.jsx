...
const Card = ({ ticketPromise, setResolved, setProgress }) => {
    ...
    if (!tickets || tickets.length === 0) {
        return <div className='text-center py-10 text-gray-400'>No tickets found</div>
    }
    return (
        <>
            <h2 className='text-xl font-bold px-7'>Customer Tickets</h2>
                        ...
                        {tickets.map(el => (
                            <div key={el.id} className='bg-white shadow rounded-lg p-4 border border-gray-200 w-full hover:shadaw-md cursor-pointer' onClick={() => handleTask(el)}>
                                <Container>
                                    ...
                                    <div className='flex justify-center items-start mt-4 text-sm'>
